...
class Discriminator(nn.Module):
    def __init__(self, state_dim, action_dim, hidden:int, disttype = "categorical"):
        ...
        self.fc1 = nn.Linear(in_features = self.state_dim+self.action_dim, out_features=self.hidden)
    ...
    def forward(self, state, action):
        if self.disttype == "categorical":
            if list(self.parameters())[0].is_cuda:
                one_hot_action = torch.cuda.FloatTensor(action.size(0) , self.action_dim).to()
            else:
                one_hot_action = torch.FloatTensor(action.size(0) , self.action_dim).to()
            one_hot_action.zero_()
            one_hot_action.scatter_(1,action.unsqueeze(1).long(),1)
