...
# self = PPOTrain(policy,old_policy)
	def train(self,  obs, actions, rewards, v_preds_next, gaes,policy_outs=None):
		...
		if self.Policy.disttype == "categorical":
			log_action_probs		= new_dist.log_prob(actions)
			old_log_action_probs	= old_dist.log_prob(actions)
		elif self.Policy.disttype == "normal":
			log_action_probs		= new_dist.log_prob(policy_outs) - torch.log(torch.clamp(1 - actions.pow(2) , 1, 1e-10 ))
			old_log_action_probs	= old_dist.log_prob(policy_outs) - torch.log(torch.clamp(1 - actions.pow(2) , 1, 1e-10 ))
			# log_action_probs		= new_dist.log_prob(policy_outs) - torch.log(torch.clamp(1 - actions.pow(2) , 1, 1e-10 ))
			# old_log_action_probs	= old_dist.log_prob(policy_outs) - torch.log(torch.clamp(1 - actions.pow(2) , 1, 1e-10 ))			
